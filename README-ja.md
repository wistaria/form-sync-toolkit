...
OAuth クライアント認証情報の取得手順は [oauth-ja.md](oauth-ja.md) を参照してください。初回の認証付きコマンド実行時に、ダウンロードした OAuth クライアント認証情報 JSON の中身を CLI に貼り付けると、`~/.config/form-sync-toolkit/credentials.json` にパーミッション `600` で保存されます。

認証情報やスコープを変更した場合は、保存済みの認証情報とトークンを削除してから再実行します。
...
- OAuth トークンは `~/.config/form-sync-toolkit/token.json` にパーミッション `600` で保存されます。
